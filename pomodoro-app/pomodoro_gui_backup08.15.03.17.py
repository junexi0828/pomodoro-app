...
class PomodoroPlannerApp:
    ...
    def _setup_styles(self):
        ...
        style.configure(
            "OnTime.TLabel",
            foreground="#87CEEB",
            font=("Helvetica", 10, "overstrike"),  # 파란색
        )
        style.configure(
            "Delayed.TLabel",
            foreground="#FFD700",
            font=("Helvetica", 10, "overstrike"),  # 노란색
        )
        style.configure(
            "Failed.TLabel",
            foreground="#FF0000",
        ...
        style.configure(
            "Pending.TLabel", foreground="#90EE90", font=("Helvetica", 10)
        )  # 초록색
    ...
    def _plot_task_status_pie(self, frame):
        ...
        ax.pie(
            ...
            autopct="%1.1f%%",
            startangle=90,
            colors=["#a6e3a1", "#f9e2af", "#f38ba8", "#a1c4fd"],
        )
        ax.axis("equal")
    ...
    def _draw_calendar_background(self):
        ...
        for hour in range(self.cal_start_hour, self.cal_end_hour + 1):
            ...
            self.calendar_canvas.create_text(
                25, y, text=f"{hour:02d}:00", anchor=tk.W, tags="background"
            )
    ...
    def _update_timer_display(self):
        if self.pomodoro_state == "paused":
            mins, secs = divmod(int(self.pause_seconds_left), 60)
            self.time_label.config(
                text=f"{mins:02d}:{secs:02d}", style="Paused.Timer.TLabel"
            )
        else:
