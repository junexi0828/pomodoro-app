...
"""
        ...
        self.theme_toggle_var = tk.BooleanVar(
            value=False
        ...
        self._apply_theme(self.current_theme_name)  # Apply initial theme here
    ...
    def _load_data_for_selected_date(self):
        ...
        self.task_id_counter = (
            max(self.displayed_tasks_data.keys()) + 1
        ...
        if self.displayed_tasks_data:
            self.next_task_id = max(
                self.next_task_id, max(self.displayed_tasks_data.keys()) + 1
            )
    ...
    def _create_left_pane_widgets(self, parent):
        ...
        analytics_button = ttk.Button(
            self.stats_frame, text="통계 분석 보기", command=self._open_analytics_window
        )
    ...
    def _draw_calendar_background(self):
        ...
        for hour in range(self.cal_start_hour, self.cal_end_hour + 1):
            ...
            self.calendar_canvas.create_line(
                50, y, width, y, fill=theme["canvas_line"], tags="background"
            )
    ...
    def _reset_pomodoro(self):
        if self.pomodoro_state == "work" or (
            self.pomodoro_state == "paused" and self.paused_from_state == "work"
        ):
    ...
    def _update_pomodoro_button(self):
        if self.pomodoro_state == "stopped":
            ...
            self.start_pause_button.config(
                text="일시 정지", command=self._toggle_pomodoro_state, state=tk.NORMAL
            )
        elif self.pomodoro_state == "break":
            if self.alarm_on:  # Alarm is still ringing
                self.start_pause_button.config(
                    text="알람 끄기", command=self._stop_alarm, state=tk.NORMAL
                )
...
""".format(
    ...
    def _apply_theme(self, theme_name):
        ...
        # Configure ttk styles for general widgets
        style = ttk.Style()
        style.theme_use("default")  # Reset to default to ensure changes apply
        style.configure(".", background=theme["bg"], foreground=theme["fg"])
        style.configure("TFrame", background=theme["frame_bg"])
        style.configure(
            "TLabelFrame", background=theme["frame_bg"], foreground=theme["fg"]
        )
        ...
        # Specific widget updates - use style for ttk widgets
        style.configure("Timer.TLabel", background=theme["bg"], foreground=theme["fg"])
        style.configure("Stats.TLabel", background=theme["bg"], foreground=theme["fg"])

        # Update task list item labels (if they exist) - ttk widgets use styles
        # No need to manually update ttk widgets as they use the configured styles above

        # Calendar canvas
        self.calendar_canvas.config(bg=theme["canvas_bg"])
        self._draw_calendar_background()  # Redraw lines and text with new colors
        self._draw_tasks_on_calendar()  # Redraw tasks with new colors
        self._update_current_time_indicator()  # Redraw current time indicator

        # Update spinboxes (they are tk.Spinbox, not ttk.Spinbox)
        self.start_hour.config(bg=theme["button_bg"], fg=theme["button_fg"])
        self.start_minute.config(bg=theme["button_bg"], fg=theme["button_fg"])
        self.end_hour.config(bg=theme["button_bg"], fg=theme["button_fg"])
        self.end_minute.config(bg=theme["button_bg"], fg=theme["button_fg"])

        # Configure ttk.Entry style
        style.configure(
            "TEntry", fieldbackground=theme["button_bg"], foreground=theme["button_fg"]
        )
    ...
    def _toggle_theme(self):
        if self.theme_toggle_var.get():  # If checkbox is checked, it's dark mode
            self._apply_theme("dark")
        else:  # If checkbox is unchecked, it's light mode
