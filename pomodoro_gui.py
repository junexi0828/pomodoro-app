...
""")
    ...
    def _quit_app(self):
        self._save_data()
        if messagebox.askokcancel("Juns Enterprise - 종료", 
...
║                                                                              ║
║  Session Summary:                                                            ║
║  • Total Focus Time: {self.today_stats.get('total_focus_seconds', 0) // 60} minutes                    ║
║  • Completed Pomodoros: {self.today_stats.get('completed_pomodoros', 0)} sessions                       ║
║  • Success Rate: {(self.today_stats.get('success', 0) / max(1, self.today_stats.get('success', 0) + self.today_stats.get('failure', 0)) * 100):.1f}% ║
║                                                                              ║
...
""".format(
                ...
                self.today_stats.get('completed_pomodoros', 0),
                (self.today_stats.get('success', 0) / max(1, self.today_stats.get('success', 0) + self.today_stats.get('failure', 0)) * 100)
            ))
            self.root.destroy()
